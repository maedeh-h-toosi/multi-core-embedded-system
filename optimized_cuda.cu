...
__device__ void forward(Net *net, Feature *features)
{
	// forward function body is allowed to modification
	int i, j, x, y, o0, o1, l0, l1;
...
__device__ int Predict(Net *net, image input)
{
...
__global__ void Cuda_Predict( Net* net,image* test_data, uint8* test_label, int* result){
...
// variables
	...
	int   *result   = (int *)calloc(COUNT_TEST, sizeof(int));
	...
	int imBytes = COUNT_TEST*sizeof(image);
	int labBytes = COUNT_TEST*sizeof(uint8);
	int netBytes = sizeof(Net);
...
//####################   CUDA   ############################
    ...
    cudaMalloc((void**)&cuda_result, COUNT_TEST*sizeof(int));
	cudaMalloc((void**)&cuda_data, imBytes);
	cudaMalloc((void**)&cuda_label, labBytes);
	cudaMalloc((void**)&cuda_net, netBytes);
	
	//Transfer data from host to device
	cudaMemcpy(cuda_data, test_data, imBytes, cudaMemcpyHostToDevice);
	cudaMemcpy(cuda_label, test_label, labBytes, cudaMemcpyHostToDevice);
	cudaMemcpy(cuda_net, net, netBytes, cudaMemcpyHostToDevice);
	
	// Set up execution configuration
 
 	// Set up execution configuration

	Cuda_Predict <<<100, 100>>> ( cuda_net ,  cuda_data, cuda_label,  cuda_result);
 
	cudaMemcpy(result, cuda_result, COUNT_TEST*sizeof(int), cudaMemcpyDeviceToHost);
...
//	gpuErrchk(cudaMemcpy(host_pred, cuda_pred, sizeof(int), cudaMemcpyDeviceToHost));
	//truePredict = *host_pred;
